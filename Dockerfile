...
RUN apk-install --no-cache --update nginx memcached php-cli php-fpm php-soap php-json php-memcache php-mysqli php-openssl php-gettext php-ctype php-xml php-phar php-dom php-curl mariadb mariadb-client strace redis inotify-tools gettext make git openssh-client perl bash jq && \
    rm -rf /var/www/* && \
